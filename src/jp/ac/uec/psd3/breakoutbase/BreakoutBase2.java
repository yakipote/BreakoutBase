 ...
 *
 */
public class BreakoutBase2 extends JPanel implements ActionListener
{
...
//
	private BufferedImage imgBlock_bomb;
	private javax.swing.Timer timer;
	static JFrame frame;

	int score = 0; 								// 得点
//
	int[][] blocks = new int[14][14];				// ブロック用配列
...
//
	private Dimension block = new Dimension( 40, 15 );	// ブロック幅・高さ
...
//
		imgBlock_bomb = getImage( "block_c_bomb.png" );
...
//
						// コンボ機能
						for(i=0; i<comb_num.length; i++){
							comb(x, y, comb_num[i][0], comb_num[i][1]);
						}
		...
		if ( sum == 0 )
		{
			//rankDialog r = new rankDialog(this.score);
			gameStat = 2;
			chk = 0;
			//s2.play();
		}
...
//
	public void comb(int x, int y, int combx, int comby)
	{
		// コンボ機能
		if(x == combx && y == comby){
			//try{
			...
			blocks[y][x-1]= 0;
			for(int i =0;i<10;i++)comb(x-1, y, comb_num[i][0], comb_num[i][1]);
			blocks[y-1][x]= 0;
			for(int i =0;i<10;i++)comb(x, y-1, comb_num[i][0], comb_num[i][1]);
			blocks[y-1][x+1]= 0;
			for(int i =0;i<10;i++)comb(x+1, y-1, comb_num[i][0], comb_num[i][1]);
			blocks[y][x+1]= 0;
			for(int i =0;i<10;i++)comb(x+1, y, comb_num[i][0], comb_num[i][1]);
			blocks[y-1][x-1]= 0;
			for(int i =0;i<10;i++)comb(x-1, y-1, comb_num[i][0], comb_num[i][1]);
...
//コンボ用爆弾表示
		for(i=0; i<comb_num.length; i++)
		{
			if(blocks[comb_num[i][1]][comb_num[i][0]] == 1)
				g.drawImage(imgBlock_bomb, 40 + w * comb_num[i][0], 55 + h * comb_num[i][1], w, h, this );
		}
	...
	public static void createFrame()
	{
		...
		frame.getContentPane().add( b3 = new BreakoutBase2() );
